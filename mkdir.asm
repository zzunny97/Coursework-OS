...
printint(int fd, int xx, int base, int sgn)
{
 ...
 41f:	f7 f6                	div    %esi
 421:	0f b6 92 dc 07 00 00 	movzbl 0x7dc(%edx),%edx
  }while((x /= base) != 0);
...
putc(int fd, char c)
{
 ...
 598:	89 45 d0             	mov    %eax,-0x30(%ebp)
        if(s == 0)
          s = "(null)";
 59b:	b8 d3 07 00 00       	mov    $0x7d3,%eax
...
free(void *ap)
{
 ...
 69a:	03 53 fc             	add    -0x4(%ebx),%edx
    ...
    p->s.ptr = bp;
  freep = p;
 69d:	a3 80 0a 00 00       	mov    %eax,0xa80
    bp->s.size += p->s.ptr->s.size;
    bp->s.ptr = p->s.ptr->s.ptr;
  } else
    bp->s.ptr = p->s.ptr;
...
malloc(uint nbytes)
{
 ...
 774:	eb e9                	jmp    75f <malloc+0xaf>
  ...
  if((prevp = freep) == 0){
    base.s.ptr = freep = prevp = &base;
 776:	c7 05 80 0a 00 00 84 	movl   $0xa84,0xa80
 77d:	0a 00 00 
 780:	c7 05 84 0a 00 00 84 	movl   $0xa84,0xa84
 787:	0a 00 00 
    base.s.size = 0;
 78a:	b8 84 0a 00 00       	mov    $0xa84,%eax
 78f:	c7 05 88 0a 00 00 00 	movl   $0x0,0xa88
 796:	00 00 00 
